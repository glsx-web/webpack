  ...
  actions: {
    ...
    load({ state, commit, dispatch }) {
      return new Promise(async resolve => {
        ...
        // 检查这个主题在主题列表里是否存在
        let __theme = state.list.find(e => e.name === activeName)
        if (__theme) {
          state.activeName = activeName
        } else {
          __theme = state.list[0]
          state.activeName = __theme.name
          // 持久化
          await dispatch('d2admin/db/set', {
            dbName: 'sys',
            path: 'theme.activeName',
        ...
        const color = await dispatch('d2admin/color/loadFromDB', null, { root: true })
        // 设置ui 颜色
        await dispatch('d2admin/color/set', color || __theme.color, { root: true })
        // 将 vuex 中的主题应用到 dom
        commit('dom')
        // end
        resolve()
    ...
    //   state.list = setting.theme.list
    //   state.activeName = setting.theme.list[0].name
    // }
  },
  mutations: {
    /**
