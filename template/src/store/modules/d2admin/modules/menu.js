...
function supplementMenuPath(menu) {
  return menu.map(e => ({
    ...e,
    path: e.path || _.uniqueId('d2-menu-empty-'),
    ...e.children ? {
...
export default {
  ...
  actions: {
    ...
    has({ state, dispatch }, path) {
      return new Promise(async resolve => {
        const _in = state.aside.some(obj => {
          if (obj.children) {
            return obj.path === path || obj.children.some(child => child.path === path)
          }
    ...
    reset({ state }) {
      state.header = []
      state.aside = []
      state.asideCollapse = setting.menu.asideCollapse
    }
  },
  mutations: {
    /**
